...
**v2.1 변경 요지**
- (2026-09-07) **§10 UI/UX 개정**: 저장소 목표(`GOAL.md`: 설계 초반의 빠른 개략 해석, 1건 ≤ 30분, 정밀 대비 오차 ≤ 10%)와 실제 구현된 노드 그래프 UI를 반영. 사용자상(비전문 설계자)·수용 기준 개정.
...
## 10. UI/UX 요구사항 — FR-11 웹 UI (2026-09-07 개정: 앞단 목표·노드 그래프 반영)

> 개정 이유. 8월의 §10은 "제출·모니터·결과 3면"을 전제로 썼다. 그 뒤 실제로
> 만들어진 UI는 **노드 그래프**(형상 → 캡 → 벤트 → 솔버 → 결과)이고, 저장소의
> 목표가 **"설계 초반에 전문 해석 지식 없이 빠르고 쉽게 개략적인 감을 잡는
> 도구"**(`GOAL.md`: 1건 ≤ 30분, 정밀 대비 오차 ≤ 10%)로 명시됐다. §10은 그 두
> 가지를 기준으로 다시 쓴다. ADR-07(구 자산 반입 금지)은 그대로다.

### 10.0 사용자상 — 누가, 언제, 무엇을 얻으려 쓰는가

| | 내용 |
|---|---|
| 누가 | 셀 **설계자**. 해석 전문가가 아니다. 메쉬·접촉·시간 간격이라는 말을 몰라도 써야 한다 |
| 언제 | 설계 **초반**. 치수·두께·벤트 규격이 아직 정해지지 않았고, 바꿔 가며 돌려 본다 |
| 무엇을 | 숫자 하나가 아니라 **경향과 판단**: "이 벤트는 0.3~0.4 MPa에서 열린다, 목표 범위 안이다", "두께를 0.1 mm 줄이면 압착 하중이 얼마나 떨어지나" |
| 얼마나 기다리나 | 노트북에서 **30분 안**. 그 이상은 앞단 도구가 아니다 |

### 10.1 원칙 — "쉽고, 빠르고, 직관적으로" (목표에 맞게 다시 씀)

1. **쉽다 — 전문 지식을 요구하지 않는다.** 기본 흐름은 노드를 잇고 실행 버튼을 누르는 것뿐이다. 모든 파라미터에 **설계자가 아는 단위의 기본값**이 채워져 있고(반경·높이·두께 mm, 벤트 길이·폭, 포일 두께), 해석 파라미터(메쉬 크기·램프·접촉·질량 스케일링)는 **사용자가 만지지 않아도 30분·10% 예산 안에 들도록 파이프라인이 정한다.** 고급 패널은 있어도 기본 화면에는 보이지 않는다.
2. **빠르다 — 기다림이 곧 비용이다.** 실행 전에 **예상 회전 시간**을 보여주고, 30분을 넘길 설정이면 실행 전에 알려준다(요소 수·최소 요소변에서 추정). 화면 자체는 첫 로드 2초, 인터랙션 100 ms, 무거운 계산은 서버(ADR-02). 결과 뷰어는 완성된 파일만 읽는다.
3. **직관적이다 — 화면이 곧 모델이고 곧 상태다.** 노드 그래프가 "무엇이 무엇에 붙어 있는가"(캔에 캡이 용접되고 캡에 벤트가 붙는다)를 그대로 보여준다. 압착 해석은 캔 노드만, 벤팅 해석은 캡·벤트 노드가 더 붙는다. 실행 중에는 결과 노드가 단계(메싱→덱→솔버→후처리)와 게이트 판정을 표시하고, 실패하면 **원인과 다음 행동을 한 문장**으로 쓴다. 전문 용어는 뷰어가 풀어 쓴다(예: "쉘 중립면 표시 — 두께는 물성으로 반영됨").
4. **경향을 보여준다, 숫자를 던지지 않는다.** 결과 화면의 1순위는 곡선과 마일스톤(파단 개시, 벤트 개방 25 %)이지 소수점 넷째 자리가 아니다. 같은 그래프에서 파라미터를 바꾼 런들을 나란히 놓을 수 있어야 한다.
5. **정지보다 신뢰 등급.** 물리적으로 틀린 모델(이중 벽, 미연결 조각, 질량 보존 ±10 % 밖)만 실행을 멈춘다. 그 밖의 게이트(에너지 오차, 메쉬 품질 경계, 미검증 재료)는 **결과와 함께 등급으로** 보여준다. 앞단 사용자가 빈손으로 돌아가는 일을 최소화한다.

### 10.2 화면 구성 — 노드 그래프 한 장 + 뷰어

| 화면 | 역할 | 핵심 요소 (현재 구현 기준) |
|---|---|---|
| **그래프** (`/`) | 모델 정의·실행·관찰 | 노드: 형상(파라메트릭 캔·각형 캔·STEP), 캡(용접 뚜껑), 벤트(포일+스코어, 패턴 선택), 하중, 메쉬, 솔버, **결과 노드**(런 목록·게이트 배지·뷰어/리포트 링크). 그래프 저장·불러오기(`configs/graphs/*.json`). 실행 큐는 직렬, 취소 가능 |
| **뷰어** (`/api/runs/{name}/viewer`, `viewers/*.html`) | 판독 | 변형 애니메이션(재생 버튼), 압력/하중-시간 곡선 + 마일스톤 마크 + 개구 면적 오버레이, **주요 치수 표**, 파트 색·스코어 강조, 모바일·태블릿 핀치/팬, WebGL 불가 시 안내 문구 |
| 리포트 (`/api/runs/{name}/report`) | 근거 | 게이트 표, 에너지 수지, 재료·메쉬 설정, 변형 형상 STEP 내보내기 |

3면 원칙은 유지된다(그래프·뷰어·리포트). "제출·모니터"는 별도 화면이 아니라 그래프의 솔버·결과 노드가 맡는다.

### 10.3 기술·디자인 제약

- 백엔드는 코어 파이프라인을 감싸는 얇은 API(`crushsim/ui/server.py`: cases·runs·graphs·viewer·report). UI는 이 API만 본다.
- 프런트는 의존성 없는 단일 HTML(`crushsim/ui/static/index.html`, 뷰어 템플릿). 빌드 도구 없이 열린다 — 노트북에서 `csim ui` 한 줄로 뜨는 것이 앞단 도구의 조건이다.
- **구 저장소의 컴포넌트·디자인 토큰·레이아웃은 어떤 형태로도 반입 금지**(ADR-07). 디자인은 엔지니어링 도구다운 밀도와 위계를 갖춘 독자 디자인. 뉴로모피즘 배제.
- 모바일·태블릿에서 뷰어는 열람·재생·확대가 되어야 한다(제출은 데스크톱 우선). 뷰어 HTML 한 장은 16 MB 이하(공유·아티팩트 한계).
- JS를 고치면 `node --check`, 뷰어 생성기를 고치면 기존 `viewers/`가 그대로 열리는지 확인.

### 10.4 UI 수용 기준 (앞단 목표 기준으로 개정)

- [ ] **전문 지식 없는 사용자**가 안내 없이 **3분 내** 첫 벤팅 해석(각형 캔 + 벤트)을 제출할 수 있다 — 기본값만으로.
- [ ] 실행 전에 예상 회전 시간이 표시되고, **30분 초과 설정은 실행 전에 경고**된다.
- [ ] 결과 화면만 보고 **설계 판단 한 문장**을 읽어낼 수 있다: "벤트 개방 압력 0.385 MPa, 목표 범위(0.3~0.5) 안" 같은 형태로 마일스톤·범위·판정이 함께 표시된다.
- [ ] 파라미터 하나를 바꾼 두 런의 곡선을 **같은 그래프에 겹쳐** 볼 수 있다.
- [ ] 실패한 런은 결과 노드에 **원인 + 다음 행동** 한 문장이 표시된다(게이트 이름만 표시하는 것은 미달).
- [ ] 정지가 아닌 게이트 미달(에너지 오차 등)은 결과와 함께 **신뢰 등급**으로 표시된다.
- [ ] 진행 상태 5초 이내 주기 갱신, 뷰어는 서버 산출 파일만으로 동작(클라이언트 물리·파싱 없음).
- [ ] 모바일·태블릿에서 뷰어 열람·재생·확대 가능.
- [ ] 구 저장소 코드·디자인 자산 유입 0건(리뷰 체크리스트).

현재 충족 상태(2026-09-07): 3분 제출·예상 시간·설계 판단 문장·곡선 겹치기·신뢰 등급은 **미충족**, 나머지는 충족. 미충족 항목이 Phase 3의 남은 일이다.
