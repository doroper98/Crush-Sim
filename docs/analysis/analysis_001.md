...
> **정정 (analysis_003)**: 이 문서는 fetch하지 않은 로컬에서 썼다. §1.2의
> "처음 확인한 미해결 문제"는 사실 EXP-004(8-31)가 이미 찾아 `geometry/skin.py`·
> `meshing/assembly.py`를 만든 결함이고, 미해결이었던 것은 **`kind: step`
> 케이스 경로가 그 모듈을 쓰지 않았다**는 점이다. §7 "어셈블리 미투입"도
> `can/test.stp`로 EXP-005가 완주했다(케이스 kind로는 아직 없음). 질문 Q1~Q7은
> 그대로 유효하다. 자세한 정정은 `analysis_003.md` §1.
